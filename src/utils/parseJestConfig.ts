...
export const parseJestConfig = (workingDirectory: string): Promise<unknown> => {
  const possibleJestConfigs: PossibleConfiguration[] =
    JEST_CONFIG_EXT_ORDER.map((extension) => ({
      path: resolve(workingDirectory, JEST_CONFIG_BASE_NAME.concat(extension)),
    }))
